import processing.video.*; //LIBRERIA DE LA CAMARA (VIDEO LIBRARY FOR PROCESSING 4)
import controlP5.*;
import oscP5.*;
import netP5.*;
Capture camara; // DECLARO LA CAMARA
ControlP5 cp5;
OscP5 oscControl;
NetAddress myRemoteLocation;
int intervalTime = 83;
int prevTime = 0;
String ipInput = "127.0.0.1"; // IP LOCAL (CAMBIAR)
int puertoInput = 3333;
...
float valor3Real = 0;
float valor4Real = 0;
String canal1_Input = "/wimumo001/emg";
String canal2_Input = "/wimumo001/emg";
String canal3_Input = "/wimumo001/emg";
String canal4_Input = "/wimumo001/emg";
...
int posXCamara, posYCamara;
void setup() {
  sessionName = d+"_"+mo+"_"+y+"-"+h+"_"+m+"_"+s+"_"+ds+"_"+cs+"_"+ms;
  camara= new Capture(this, 640, 480); //TAMANIO DE LA CAMARA
  camara.start();//INICIALIZO LA CAMARA
  posXCamara=1000;
  posYCamara=110;
  table = new Table();
  ...
  myRemoteLocation = new NetAddress("127.0.0.1", puertoInput); // IP LOCAL (CAMBIAR)
  ...
  cp5.addTextfield("canal1_Input")
    ...
    .setAutoClear(false)
    ;
  cp5.addTextfield("UBICACION1")
    .setPosition(1000, 135)
    .setSize(150, 20)
    ;
  //2
  cp5.addTextfield("canal2_Input")
    ...
    .setAutoClear(false)
    ;
  cp5.addTextfield("UBICACION2")
    .setPosition(1000, 340)
    .setSize (150, 20)
    ;
  ...
  cp5.addTextfield("canal3_Input")
    ...
    .setAutoClear(false)
    ;
  cp5.addTextfield("UBICACION3")
    .setPosition(1000, 540)
    .setSize (150, 20)
    ;
  ...
  cp5.addTextfield("canal4_Input")
    ...
    .setAutoClear(false)
    ;
  cp5.addTextfield("UBICACION4")
    .setPosition(1000, 740)
    .setSize (150, 20)
    ;
...
void draw() {
  ...
  camara.read(); //LEE LA CAMARA
  image(camara, posXCamara, posYCamara); //MUESTRA LA CAMARA
}
